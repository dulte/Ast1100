...
class satelite:
    def __init__(self,delta_v,destination):
        ...
        self.position = np.array([self.planetPosFunction(self.time_to_launch)[0,0],self.planetPosFunction(self.time_to_launch)[1,0]])
        v0 = [-np.sin(self.calc_tangental_angle(self.time_to_launch))*(self.dv_mainburn),np.cos(self.calc_tangental_angle(self.time_to_launch))*(self.dv_mainburn)]
        print v0
    ...
    def acceleration(self,planetPos):
        ...
        rx = (planetPos[0,:] - self.position[0])
        ry = (planetPos[1,:] - self.position[1])

        some = norm(np.array([rx,ry]),axis = 0)
        acceleration[0] = np.sum((self.G*self.planetMasses[:]/(some**3)*rx))
        acceleration[1] = np.sum((self.G*self.planetMasses[:]/(some**3)*ry))
    ...
    def main_loop(self):
        ...
        for i in xrange(1,self.steps_per_year*self.sim_time-1):
            ...
            if (abs(self.calc_dist_to_planet(self.destination_planet,time[-1],self.position)) < min_dist_to_planet):
                min_dist_to_planet = abs(self.calc_dist_to_planet(self.destination_planet,time[-1],self.position))
        ...
        #plt.plot(self.planetPosFunction(self.))
