...
class satelite:
    def __init__(self,delta_v,destination):
        ...
        self.sim_time = 3
        self.steps_per_year = 200000#365*24*60#300000
    ...
    def main_loop(self):
        ...
        #while (time[-1] < self.steps_per_year*self.sim_time):
            ...
            if (i%1000 == 0):
                ...
                if r_home > 0.005 and not lauched:
                    ...
                    lauched = True
                elif r_dest < 0.005 and not close_to_dest:
                    print "Getting close to destinaton"
                    close_to_dest = True
                elif r_dest > 0.005 and close_to_dest:
        ...
        #
        #     time[i] = time[i-1] + dt
        #     self.velocity += self.acceleration(self.planetPosFunction(time[i]))*dt
        #
        #
        #     if ((i+1)%self.writeingFreq == 0 ):
        #         self.pos_over_time[:,i/self.writeingFreq+1] = self.position
        #         time_save.append(time[i])
        #
        #
        #
        #     if (i%1000 == 0):
        #         r_home = (self.calc_dist_to_planet(self.home_planet,time[i-1],self.position))
        #
        #         if (r_home >= 0.01):
        #             lauched = True
        #             print "The satelite has now lauched"
        #             break
        #
        #
        #
        ...
        #
        # else:
        #     print "Failed to get away for home"
        #     sys.exit(1)
        #
        #
        # self.velocity -= 0.5*self.acceleration(self.planetPosFunction(time[couter-1]))*self.dt
        # dt = self.dt
        # self.velocity += 0.5*self.acceleration(self.planetPosFunction(time[couter-1]))*self.dt
        #
        # for i in xrange(couter+1,self.steps_per_year*self.sim_time):
        #
        #     couter += 1
        #     self.position += self.velocity*dt
        #
        ...
        #     if (r_dest < min_dist_to_planet):
        #         min_dist_to_planet = r_dest# abs(self.calc_dist_to_planet(self.destination_planet,time[-1],self.position))
        #         time_closest_incounter = time[i]
        #         index_closest_incounter = i
        #
        #
        #     if (i%1000 == 0):
        #         r_dest = (self.calc_dist_to_planet(self.destination_planet,time[i-1],self.position))
        #
        #         if (r_dest < 0.01 ):
        #             close_to_dest = True
        #             print "You are now close to the destinaton planet"
        #             break
        #
        #
        #
        # else:
        #     print "Failed to get close to destinaton"
        #
        #
        #
        # self.velocity -= 0.5*self.acceleration(self.planetPosFunction(time[couter-1]))*self.dt
        ...
        #             if (r_dest >= 0.01 ):
        #                 close_to_dest = True
        #                 print "You are now going away from the planet"
        #                 break
        #
        #
        #         if (r_dest < min_dist_to_planet):
